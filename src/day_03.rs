...
impl Point {
    fn move_point(self, dir: char) -> Point {
        match dir {
            'R' => {
                return Point {
                    x: self.x + 1,
...
impl Wire {
    fn extend_wire(&mut self, command: Command) {
        for _ in 0..command.length {
            self.points.push(self.current.move_point(command.direction));
            self.current = self.current.move_point(command.direction);
        }
    }
}
...
impl std::fmt::Debug for Command {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "dir: {}  len: {}", self.direction, self.length)
    }
}

fn find_intersections(wire1: Wire, wire2: Wire) {
    let mut min_dist = 100000000;
    let mut wire1_points = HashMap::new();
    for p in wire1.points {
        wire1_points.insert(p.val(), 1);
    }
    for p in wire2.points {
        match wire1_points.get(&p.val()) {
            Some(&_) => {
                let dist = p.x.abs() + p.y.abs();
                if dist < min_dist {
                    println!("Intersect A: {:?} dist {}", p, dist);
                    min_dist = dist
                }
            }
            _ => {}
...
fn find_intersections_b(wire1: Wire, wire2: Wire) {
    let mut min_dist = 100000000;
    let mut wire1_points = HashMap::new();
    let mut index = 0;
    for p in wire1.points {
        ...
        match wire1_points.get(&p.val()) {
            Some(&_) => {}
            _ => {
                wire1_points.insert(p.val(), index);
            }
        }
    }
    index = 0;
    for p in wire2.points {
        ...
        match wire1_points.get(&p.val()) {
            Some(&wire1_dist) => {
                let dist = wire1_dist + index;
                if dist < min_dist {
                    println!("Intersect B: {:?} dist {}", p, dist);
                    min_dist = dist
                }
            }
            _ => {}
...
pub fn day3b() {
    ...
    find_intersections_b(wire1, wire2);
}
